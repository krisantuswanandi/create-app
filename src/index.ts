  ...
  if (projectName) {
    ...
    try {
      const response = await prompts({
        ...
        validate: (val: string) => val.includes(' ') ? 'Cannot contain space' : true
      })
...
function copyDir(srcDir: string, destDir: string, options?: Record<string, string>) {
...
getProjectName().then(projectName => {
  if (projectName) {
    const templatePath = path.join(url.fileURLToPath(import.meta.url), '../..', 'template')
